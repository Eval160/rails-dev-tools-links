class ResourcesController < ApplicationController
  ...
  def index
    @resources = Resource.includes(:categories, :user, photo_attachment: :blob)
    p = params["/resources"]
    if !params["/resources"].nil? && p[:category].length != 1 && p[:category]
      resources = Resource.joins(:category_tags).includes(:categories, :user, photo_attachment: :blob).where(category_tags: { category: p[:category] })
  ...
  def user_resources
    @resources = current_user.resources.includes(:categories, :user, photo_attachment: :blob)
  end
